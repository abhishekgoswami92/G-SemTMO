"""
Ablation 1 is a training with Only FCNCT and no semantic information or Graph Convolutions
...
from CT_utils.CT_library import rgb2lab, lab2rgb
...
device = 'cuda'
model_code = 'abl1'                                                     # model code to identify the network trained
...
model_path_fcn = f'Model/Pretrained/Ablation/ablation1_FiveK.pth'       # path for fcn model
...
def broadcast_hint(segv, hints):
    """
    Needed when broadcasting node hints to all pixels in the node in a vector format (mainly in Ablation 2 and Gsemtmo)
    """
...
data_train = dl.custom_DL(datasetinfo=dataset_info[:slice_train], root_dir=dataset_root_dir)
data_val = dl.custom_DL(datasetinfo=dataset_info[slice_train:slice_val], root_dir=dataset_root_dir)
# ------------------------------------------------------------------------------------------------- Custom data loader
